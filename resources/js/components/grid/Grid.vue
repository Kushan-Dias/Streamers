<!--<template>
  <div>
    <CTable align="middle" class="mb-0 border" hover responsive>
      <CTableBody>
        <CTableRow v-for="item in tableExample" :key="item.name" class="table-row-link" tag="div">
          <! router-link for row 
          <router-link :to="`/streamer/profile/${item.name}`" tag="div" class="table-row-link">
...
export default {
  ...
  setup() {
    ...
    const navigateToProfile = (itemId) => {
      router.push(`/streamer/profile/${itemId}`);
    };
...
<template>
  <div>
    <CTable align="middle" class="mb-0 border" hover responsive>
      <CTableBody>
        <CTableRow v-for="item in tableExample" :key="item.id" class="table-row-link" tag="div">
          <!-- router-link for row -->
          <router-link :to="`/streamer/profile/${item.id}`" tag="div" class="table-row-link" @click="navigateToProfile(item.id)">
            <CTableDataCell class="text-center">
              <CAvatar
                size="md"
                :src="item.avatar.src"
